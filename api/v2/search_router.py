...
@router.get('/actors', response_model=list[ActorResult])
async def fetch_search_data(session=Depends(get_session)):
    actor_ids = session.exec(select(Actor).where(Actor.current).options(load_only(Actor.id))).all()
...
@router.get('/plays', response_model=list[PlayResult])
async def fetch_search_data(session=Depends(get_session)):
    if babel.locale != 'en':
        plays = session.exec(select(PlayTranslations)).all()
    else:
