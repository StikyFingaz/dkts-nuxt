...
async def fetch_play_photos(play_slug: str):
    ...
    # play_thumbs_dir = Path(working_dirs[working_env]) / f'media/plays/{play_slug}/photos/thumbs'
    play_images = defaultdict(dict)
    ...
    for img in sorted(play_photos, key=lambda x: int(x.split('.')[0])):
        ...
        img_format = get_image_size(img_file)
        width, height = img_format.get_dimensions()
        play_images['play_photos'][img] = {'width': width, 'height': height}

    # play_thumbs = [f for f in os.listdir(play_thumbs_dir) if os.path.isfile(f'{play_thumbs_dir}/{f}')]
    # for img in sorted(play_thumbs, key=lambda x: int(x.split('.')[0])):
    #     img_file = f'{play_thumbs_dir}/{img}'
    #     img_format = get_image_size(img_file)
    #     width, height = img_format.get_dimensions()
    #     play_images['play_thumbs'][img] = {'width': width, 'height': height}
